...
class AlmoxarifadoController extends Controller
{
    ...
    public function qrCodeGerador($id)
    {
        ...
        $qrCodeImage = QrCode::size(100)
            ->color(0, 60, 130)
            ->generate('http://siap.rbprev.riobranco.ac.gov.br/consulta/' . $id);
        return view('daf.almoxarifado.qrcode', compact('qrCodeImage', 'almo'));
    }

    public function showImagem($id)
    {
        $almoxarifado = Almo::findOrFail($id);

        $imgPath = storage_path('app/public/' . $almoxarifado->imagem);

        if (file_exists($imgPath)) {
            $fileContent = file_get_contents($imgPath);
            $type = mime_content_type($imgPath);
            $response = new \Illuminate\Http\Response($fileContent, 200, [
                'Content-Type' => $type,
                'Content-Disposition' => 'inline; filename="' . $almoxarifado->imagem . '"'
            ]);
    ...
    public function showImagem_public($id)
    {
        $almoxarifado = Almo::findOrFail($id);
